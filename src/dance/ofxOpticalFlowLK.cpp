...
void ofxOpticalFlowLK :: draw ( int width, int height,  float lineScale, int res )
{
	...
	for( int x=0; x<width; x+=res )
	{
		for( int y=0; y<height; y+=res )
		{
			...
			glVertex2f(x, y);
			glColor4f(1,1,1,0);
			glVertex2f(x + vel.x * lineScale, y + vel.y * lineScale );
			glEnd();
...
void ofxOpticalFlowLK :: draw2 ( int width, int height,  float lineScale, int res )
{
	...
	for( int x=0; x<width; x+=res )
	{
		for( int y=0; y<height; y+=res )
		{
			...
			float velLen = sqrt(vel.x*vel.x + vel.y*vel.y);
			
			float pp = (ofGetAppPtr()->mouseX)/5.0;
